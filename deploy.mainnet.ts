...
async function main() {
  ...
  if (net.chainId !== 1n) {
    throw new Error(
      `ERROR: Wrong network. Expected Mainnet (chainId=1), got ${net.chainId}`
    );
  }

  // HARD GUARD: MANUAL CONFIRMATION REQUIRED
  if (process.env.CONFIRM_MAINNET_DEPLOY !== "YES") {
    throw new Error(
      "ERROR: Refusing to deploy to mainnet. Set CONFIRM_MAINNET_DEPLOY=YES to continue."
    );
  }
  ...
  console.log("Deploying USTA to MAINNET");
  ...
  console.log(
    `npx hardhat verify --network mainnet --build-profile production ${address} ${recipientAddr}`
  );
